...
class crud
{
  ...
  {
    try {
      $this->db=new PDO('mysql:host='.$this->dbhost.';dbname='.$this->dbname.';charset=utf8',$this
      ->dbuser,$this->dbpass);
     echo "Baglanti basarili";
    } catch (\Exception $e) {
      die("Baglantı Başarısız:".$e->getMessage());
    }
  ...
  public function update($table,$values,$options=[]){
    try {
      ...
      //admins_id degerini dizide en alta alma
...
// delete islemleri
  ...
  // metodlasmıs resim yukleme kısa
  ...
  // resim yukleme bıtıs her resım yukleme alanına bunu cek...

  public function adminsInsert($admins_namesurname,$admins_username,$admins_pass,$admins_status){
    try {
